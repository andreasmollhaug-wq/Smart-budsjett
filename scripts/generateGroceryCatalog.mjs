...
const catalogOut = join(__dirname, '../src/lib/groceries/groceryCatalog.ts')
const classicOut = join(__dirname, '../src/features/matHandleliste/classicGroceries.ts')
...
const catalogHeader = `/**
 * Kuratert norsk dagligvarekatalog (~250 varer). Generert av scripts/generateGroceryCatalog.mjs
 */
import type { GroceryCatalogEntry } from './groceryCatalogTypes'

export const GROCERY_CATALOG: GroceryCatalogEntry[] = `

const catalogBody = `${catalogHeader}${JSON.stringify(entries, null, 2)}
`

writeFileSync(catalogOut, catalogBody, 'utf8')

const classicHeader = `/**
 * Klassiske norske dagligvarer — re-export fra lib/groceries.
 * Generert av scripts/generateGroceryCatalog.mjs
 */
import type { GroceryCatalogEntry } from '@/lib/groceries/groceryCatalogTypes'
import { GROCERY_CATALOG } from '@/lib/groceries/groceryCatalog'
import { filterGroceryCatalog } from '@/lib/groceries/filterGroceryCatalog'
import { normalizeProductKey } from '@/lib/groceries/normalizeProductKey'

export type ClassicGroceryEntry = GroceryCatalogEntry

export const CLASSIC_GROCERIES = GROCERY_CATALOG
...
export function filterClassicGroceries(query: string, limit = 80): ClassicGroceryEntry[] {
  return filterGroceryCatalog(query, limit)
}
`

writeFileSync(classicOut, classicHeader, 'utf8')
console.log('Wrote', catalogOut, 'and', classicOut, 'count', items.length)
