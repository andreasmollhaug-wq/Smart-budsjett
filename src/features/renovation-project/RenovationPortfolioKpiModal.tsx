...
const LABELS: Record<RenovationPortfolioKpiKind, string> = {
  activeProjects: 'Aktive hovedprosjekt',
  totalBudget: 'Samlet budsjett',
  totalActual: 'Samlet faktisk',
  checklist: 'Sjekkliste (samlet)',
}

export default function RenovationPortfolioKpiModal({ open, onClose, kind, portfolio, projects }: Props) {
  const { formatNOK } = useNokDisplayFormatters()

  const activeRoots = useMemo(() => getActiveRootProjects(projects), [projects])

  const rows = useMemo(() => {
    return activeRoots.map((p) => ({
      project: p,
      k: computeRollupProjectKpis(p, projects),
    }))
  }, [activeRoots, projects])
  ...
  const checklistRows = useMemo(() => {
    return rows.map(({ project: root, k }) => {
      const subs = getActiveChildProjects(root.id, projects).map((c) => ({
        id: c.id,
        name: c.name,
        kpis: computeProjectKpis(c),
      }))
      return { root, k, subs }
    })
  }, [rows, projects])
    ...
    <RenovationModalFrame onRequestClose={onClose} ariaLabelledBy={titleId} maxWidth="lg">
        ...
        >
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <p className="text-xs leading-snug sm:text-sm" style={{ color: 'var(--text-muted)' }}>
              Hoved og rom på eget spor — ikke synkronisert med hovedbudsjettet
            </p>
          </div>
          <button
            type="button"
        ...
        <div className={renovationModalScrollableMainClass}>
          {kind === 'activeProjects' ? (
            <div className="space-y-4">
              <p className="text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                <strong style={{ color: 'var(--text)' }}>{portfolio.activeProjectCount}</strong> aktive{' '}
                <strong>hoved</strong>
                prosjekt{portfolio.activeProjectCount !== 1 ? 'er' : ''}.
                {portfolio.activeSubprojectCount > 0 ? (
                  <>
                    {' '}
                    Under samme tak ligger også{' '}
                    <strong style={{ color: 'var(--text)' }}>{portfolio.activeSubprojectCount}</strong> aktiv
                    {portfolio.activeSubprojectCount !== 1 ? 'e' : ''} rom/underprosjekt — de inngår i hovedtalene.
                  </>
                ) : (
                  <>
                    {' '}
                    Opprett rom under «Underprosjekt» for å strukturere.
                  </>
                )}{' '}
                Arkiverte prosjekter telles ikke.
              </p>
              {rows.length === 0 ? (
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                  Ingen aktive hovedprosjekter — opprett et nytt eller koble rom til et hus.
                </p>
              ) : (
                    ...
                    <li key={project.id}>
                      ...
                      >
                        ...
                        <span className="text-xs tabular-nums" style={{ color: 'var(--text-muted)' }}>
                          ...
                          <span className="opacity-85">— hus og rom</span>
                        </span>
                      </Link>
                    </li>
                  ))}
          ...
          {kind === 'totalBudget' ? (
            <div className="space-y-4">
              <p className="text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                Summen er <strong>budsjettlinjer på hovednivå pluss på rom</strong> for alle aktive hovedprosjekter
                (dobbelttelling unngås).
          ...
          {kind === 'totalActual' ? (
            <div className="space-y-4">
              <p className="text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                Summen er alle <strong>registrerte utgifter</strong> på hovedprosjekt og rom under. Andel av samlet budsjett brukt:{' '}
          ...
          {kind === 'checklist' ? (
            <div className="space-y-4">
              <p className="text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                Fullførte punkter fra <strong>alle</strong> aktive hovedprosjekter og deres rom. Per rad: tall for hus samlet med rom; punkter på rom vises som undertekst.
              </p>
                  ...
                  {checklistRows.map(({ root, k, subs }) => (
                    <li key={root.id} className="space-y-1.5">
                      <div className="flex justify-between gap-2 text-sm min-w-0">
                        ...
                        <span className="shrink-0 tabular-nums text-xs sm:text-sm" style={{ color: 'var(--text-muted)' }}>
                          {k.checklistDone} / {k.checklistTotal} (hus og rom samlet)
                        </span>
                      </div>
                      {k.checklistTotal > 0 ? (
                      ...
                      {subs.length > 0 ? (
                        <ul className="pl-3 space-y-1 text-xs border-l mt-2" style={{ borderColor: 'var(--border)' }}>
                          {subs.map((s) => (
                            <li key={s.id} style={{ color: 'var(--text-muted)' }}>
                              {s.name}: {s.kpis.checklistDone} / {s.kpis.checklistTotal}
                            </li>
                          ))}
                        </ul>
                      ) : null}
