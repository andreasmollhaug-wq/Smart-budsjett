...
export default function OnboardingWizard() {
  ...
  useEffect(() => {
    ...
    const cat = st.people[st.activeProfileId]?.budgetCategories.find(
      (c) => c.id === ONBOARDING_MAIN_INCOME_CATEGORY_ID,
    )
  ...
  const goNext = () => {
    ...
    if (step === 3) {
      ...
      updateBudgetCategory(ONBOARDING_MAIN_INCOME_CATEGORY_ID, { budgeted })
      ...
      >
        ...
        {step === 3 && (
          <div>
            ...
            <p className="text-sm mt-2" style={{ color: 'var(--text-muted)' }}>
              Omtrentlig beløp etter skatt for linjen «Lønn» i budsjettet. Du kan justere detaljene senere.
            </p>
            <label className="block mt-4 text-sm font-medium" style={{ color: 'var(--text)' }}>
              Beløp (kr)
              <input
