...
export default function DashboardSavingsGoalsModal({ open, onClose, savingsGoals }: Props) {
  useEffect(() => {
    if (!open) return
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="dashboard-savings-modal-title">
                ...
                return (
                  <div key={g.id} className="space-y-2">
                    ...
                    <div className="flex justify-between text-xs" style={{ color: 'var(--text-muted)' }}>
                      <span>{formatNOK(g.currentAmount)}</span>
