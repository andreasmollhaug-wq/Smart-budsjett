...
Demodata
- Slås på/av under startveiledning eller Min konto → Innstillinger (seksjonen «Demodata»). Når demodata er på, vises eksempeldata for blant annet budsjett, transaksjoner, sparemål, investeringer og lån; **smartSpare** (/sparing/smartspare) får **én eksempelplan per budsjettprofil** med månedstabell og innbetalinger tilpasset demovarianten og aktivt budsjettår (hvert medlem i familiehusholdning med demodata har sin egen plan-ID); profiler uten demodata eller uten aktiv demo har ingen eksempelplan til du oppretter selv; på SmartVane opprettes også forhåndsutfylte demovaner per aktiv budsjettprofil (navn med prefiks «Demo · » lagret i databasen) med kryss i forrige og inneværende måned — de ryddes når demodata slås av for den profilen med mindre du allerede hadde registrert egne vaner på den profilen (da hopper demostrøyen for SmartVane over for å ikke blande); egne øvrige data lagres og gjenopprettes når demodata slås av. I familiehusholdning med flere profiler er eksempeldata ulike per medlem (første profil følger standardsettet), og SmartVane-vaner følger samme profil som «Viser data for» (cookies synker profil for innloggede sider).
