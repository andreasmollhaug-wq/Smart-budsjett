...
/**
 * Kun for «Bruk eksisterende linje» på abonnement-siden: deler Regninger-linjer i Streaming vs Abonnementer.
...
/**
 * Hvis brukeren allerede har valgt en Regninger-linje som ikke er streaming/abonnement (eldre data),
...
export function subscriptionSharedLineLegacyCategory(
  ...
  const inPartition = new Set(
    [...partition.streaming, ...partition.abonnement].map((c) => c.id),
  )
